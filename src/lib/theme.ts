...
import vidro from '../data/themes/vidro.json';
import agencia from '../data/themes/agencia.json';
...
export const PRESETS: Record<string, ThemePreset> = { dental, harmonizacao, vidro, agencia };
...
export function resolveTheme(): { vars: Record<string, string>; preset: ThemePreset } {
  const preset = PRESETS[themeConfig.preset] ?? PRESETS.vidro;
